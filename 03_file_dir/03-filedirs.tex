...
\begin{itemize}
\item if \textit{newname} exists and is an empty directory, it is removed; \textit{oldname} is renamedd \textit{newname}
\item if \textit{newname} exists and is a file, an error is returned
\item if \textit{oldname} is a prefix of \textit{newname}, an error is returned
...
\begin{itemize}
\item \textbf{\textit{oldname} or \textit{newname} is symbolic link}, then the link itself is processed, not the file to which it resolves
\item We can’t rename dot or dot-dot.
\item As a special case, if \textit{oldname} and \textit{newname} refer to the same file, the function returns successfully without changing anything.
\end{itemize}
